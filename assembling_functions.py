...
def assemble_state_transitions_and_boolean_conditions(starting_state: State, finishing_state: State, wanted_value_condition=True) -> Condition :
    condition = ValueCondition(False, wanted_value_condition)
    transition = MonitoredTransition(finishing_state, condition)
    starting_state.add_transition(transition)
    return condition
